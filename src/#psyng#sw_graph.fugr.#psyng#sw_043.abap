...
*"  TABLES
*"      IT_LOCHDR STRUCTURE  /PSYNG/SW_LOCHDR OPTIONAL
*"  EXCEPTIONS
*"      NO_DATA_TO_COMPRESS
*"      COMPRESS_FAILURE
...
*"----------------------------------------------------------------------
  ...
  DATA:
    ...
    lt_hc_hdr2 TYPE STANDARD TABLE OF /psyng/sw_hc_hdr WITH HEADER LINE,
  ...
  IF it_lochdr[] IS INITIAL.
    SELECT * FROM /psyng/sw_lochdr
             INTO CORRESPONDING FIELDS OF TABLE lt_lochdr
  ...
  FREE: it_lochdr.
  SORT lt_lochdr BY vrsio sysid.
  IF lt_lochdr[] IS INITIAL.
  ...
  DESCRIBE TABLE lt_lochdr LINES records_uncomp.
  ...
  ENDIF.
* get the data from the lochdr table
  READ TABLE lt_lochdr INDEX 1 TRANSPORTING erdat .
  l_date = lt_lochdr-erdat.
  CLEAR:lt_lochdr.
  FREE: lt_lochdr.

  CLEAR: lt_hc_hdr-counter.
  LOOP AT lt_compressed ASSIGNING <cmp>.
    lt_hc_hdr-mandt = sy-mandt.
    lt_hc_hdr-sysid = sysid.
    lt_hc_hdr-vrsio = vrsio.
    lt_hc_hdr-counter = lt_hc_hdr-counter + 1.
    lt_hc_hdr-cdate = l_date.
    lt_hc_hdr-data = <cmp>.
    APPEND lt_hc_hdr.
  ENDLOOP.
  CLEAR: lt_compressed.
  ...
  DESCRIBE TABLE lt_hc_hdr LINES records_comp.
  l_comp_recordsize = sy-tleng.

  IF delete_first = 'X'.
    DELETE FROM /psyng/sw_hc_hdr WHERE cdate = l_date AND
                                       sysid = sysid  AND
                                       vrsio = vrsio.
    COMMIT WORK.
  ENDIF.
  ...
  CLEAR lf_subrc.
  SORT lt_hc_hdr.
  WHILE NOT lt_hc_hdr[] IS INITIAL.
    APPEND LINES OF lt_hc_hdr FROM 1 TO lf_commitcount TO lt_hc_hdr2.
    DELETE lt_hc_hdr FROM 1 TO lf_commitcount.
    MODIFY /psyng/sw_hc_hdr FROM TABLE lt_hc_hdr2.  "#EC CI_IMUD_NESTED
    COMMIT WORK.
  ...
  CLEAR: lt_lochdr, lt_lochdr2, lt_hc_hdr, lt_compressed,
         l_uncomp_recordsize, l_comp_recordsize.
  FREE: lt_lochdr, lt_lochdr2, lt_hc_hdr, lt_compressed.
...
"(++)EOC UMITTAL SE VF scan-25/11/2024.
  ...
  SELECT * FROM /psyng/sw_lochdr     "#EC CI_NOWHERE
           INTO CORRESPONDING FIELDS OF TABLE lt_lochdr2
           WHERE sysid = sysid AND vrsio = vrsio.
  sort : lt_lochdr2, lt_lochdr.
  IF lt_lochdr2[] = lt_lochdr[].
    MESSAGE s398(00) WITH
