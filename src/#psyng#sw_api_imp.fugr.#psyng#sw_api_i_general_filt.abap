...
*----------------------------------------------------------------------*
FUNCTION /psyng/sw_api_i_general_filt.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"     VALUE(NO_OF_DAYS) TYPE  /PSYNG/SE_RETENTION_DAYS OPTIONAL
*"     VALUE(END_DATE) TYPE  /PSYNG/END_DATE OPTIONAL
*"     VALUE(START_DATE) TYPE  /PSYNG/START_DATE OPTIONAL
*"     VALUE(ORG_CHK) TYPE  FLAG OPTIONAL
*"     VALUE(CROSS_CHK) TYPE  FLAG OPTIONAL
*"     VALUE(REM_CHK) TYPE  FLAG OPTIONAL
*"     VALUE(LOCAL_CHK) TYPE  FLAG OPTIONAL
*"     VALUE(REMONL_CHK) TYPE  FLAG OPTIONAL
*"     VALUE(NOCROSS_CHK) TYPE  FLAG OPTIONAL
*"  EXPORTING
...
*"----------------------------------------------------------------------
  ...
  DATA : lt_return     TYPE TABLE OF  bapiret2 WITH HEADER LINE,
         ...
         lt_var_val    TYPE TABLE OF rsparams WITH HEADER LINE,
         ...
         lv_dat_low    TYPE /psyng/swreshdr-start_date,
...
*--Get Information about the latest published Configuration Set
  ...
  IF no_of_days IS NOT INITIAL.
    CLEAR lv_dat_low.
    lv_dat_low = sy-datum - no_of_days.
    DELETE et_reshdr WHERE start_date < lv_dat_low.
  ELSEIF start_date IS NOT INITIAL OR end_date IS NOT INITIAL.
    IF start_date IS NOT INITIAL.
      DELETE et_reshdr WHERE start_date < start_date.
    ENDIF.
    IF end_date IS NOT INITIAL.
      DELETE et_reshdr WHERE end_date > end_date.
    ENDIF.
  ENDIF.

  CLEAR ls_ana_header.
  LOOP AT et_reshdr INTO ls_ana_header.
    CLEAR lt_var_val[].
    CALL FUNCTION 'RS_VARIANT_CONTENTS'
      EXPORTING
        report                      = '/PSYNG/SW_140'
        variant                     = ls_ana_header-variant_name
      TABLES
        valutab                     = lt_var_val
     EXCEPTIONS
    ...
    IF sy-subrc <> 0.
      ...
      CASE sy-subrc.
        WHEN 1.
          ls_message-message = 'Variant does not exist'(008).
          MESSAGE ls_message-message TYPE 'S'.
        WHEN 2.
          ls_message-message = 'Report does not exist'(009).
          MESSAGE ls_return-message TYPE 'S'.
        WHEN 3.
          ls_message-message = 'An unknown error occured'(010).
          MESSAGE ls_message-message TYPE 'S'.
      ENDCASE.
* Implement suitable error handling here
    ELSE.
      READ TABLE lt_var_val WITH KEY selname = 'ORGCHK'
                                     low     = org_chk.
      IF sy-subrc = 0.
        READ TABLE lt_var_val WITH KEY selname = 'P_CROSS'
                                    low     = cross_chk.
        IF sy-subrc = 0.
          READ TABLE lt_var_val WITH KEY selname = 'P_REMOTE'
                                  low     = rem_chk.
          IF sy-subrc = 0.
            READ TABLE lt_var_val WITH KEY selname = 'P_LOCAL'
                                  low     = local_chk.
            IF sy-subrc = 0.
              READ TABLE lt_var_val WITH KEY selname = 'P_REMON'
                                  low     = remonl_chk.
              IF sy-subrc = 0.
                READ TABLE lt_var_val WITH KEY selname = 'P_NOCRO'
                                  low     = nocross_chk.
                 IF sy-subrc <> 0.
                   DELETE et_reshdr WHERE aid = ls_ana_header-aid.
                 ENDIF.
              ELSE.
                DELETE et_reshdr WHERE aid = ls_ana_header-aid.
              ENDIF.
...
*--Return the systems inside the configuration set(s)
  SELECT * FROM /psyng/swcfgsys INTO TABLE et_set_systems  WHERE setid =
  es_config_set-setid.
  IF et_set_systems IS REQUESTED.
    IF et_config_set[] IS NOT INITIAL.
    SELECT * FROM /psyng/swcfgsys APPENDING TABLE et_set_systems FOR ALL
    ENTRIES IN et_config_set WHERE setid = et_config_set-setid.
    ENDIF.
  ENDIF.
