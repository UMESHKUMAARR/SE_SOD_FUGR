...
********************************
****RK28112011 Declaration for GRAPH_MATRIX_3D
...
if ls_config-value = 'Y'.
  CALL FUNCTION 'POPUP_TO_DECIDE'
    EXPORTING
      ...
      text_option1            = TEXT-015 "'Standard SOD-Matrix'(015)
      text_option2            = TEXT-016 "'Enhanced SOD-Matrix'(016)
...
*SELECT * FROM (ls_tabname) INTO TABLE syscandt  WHERE
*       conid <> 'NONE'
*       AND   vrsio =  sodvrsio.
...
LOOP AT syscandt.
  ...
  CLEAR gf_reject.
*DHO 20101202
*  PERFORM check_ugroup_auth USING users-class
*                            CHANGING gf_reject.
  ...
  IF users-class IS INITIAL.
    users-class = 'N/A'.
  ENDIF.
  READ TABLE info WITH KEY usergroup = users-class.
  IF sy-subrc = 0.
    ...
    CASE conflict-imp.
      WHEN 'HIGH'.
        ...
        MODIFY info TRANSPORTING high WHERE usergroup = users-class.
      WHEN 'MEDIUM'.
        info-medium = info-medium + 1.
        MODIFY info TRANSPORTING medium WHERE usergroup = users-class.
      WHEN 'LOW'.
        info-low = info-low + 1.
        MODIFY info TRANSPORTING low WHERE usergroup = users-class.
      WHEN 'CRITICAL'.
        info-critical = info-critical + 1.
        MODIFY info TRANSPORTING critical WHERE usergroup = users-class.
      WHEN OTHERS.
    ENDCASE.
  ELSE.
    IF users-class IS INITIAL.
