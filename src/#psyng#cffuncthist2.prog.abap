REPORT /psyng/cffuncthist NO STANDARD PAGE HEADING .
**----------------------------------------------------------------------
*
...
DATA: gs_program LIKE sy-repid,            "For ALV call
      gs_variant type disvariant.       "Changed by sgottapu on
                                        "16-09-2008.
...
*----------------------------------------------------------------------*
FORM get_history.
  SELECT * FROM /psyng/history WHERE tabname = '/PSYNG/CONFLICT'
                                        AND   oldval  IN conid
    ...
    APPEND ihistory.
    SELECT * FROM /psyng/history WHERE tabname = '/PSYNG/CONFDET'
