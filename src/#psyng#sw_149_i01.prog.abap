...
*--check warning msg for config set & versio
  IF g_preconfig <> /psyng/swrrshdr-setid.
    CLEAR g_warning_msg.
  ENDIF.
...
*--Check Config Set
    g_warning_msg = 'X'.
    g_preconfig = /psyng/swrrshdr-setid.

    CALL FUNCTION '/PSYNG/SW_CGF_SET_VALID'
         EXPORTING
              i_setid         = /psyng/swrrshdr-setid
              i_vrsio         = /psyng/swrrshdr-sodvrsio
              if_show_warning = 'X'.
  ENDIF.
ENDMODULE.
...
* EOC by RGUPTA on 28.03.22 for C0700
  CASE sy-ucomm.
    WHEN 'MINE'.
      CLEAR /psyng/swrrshdr-bname.
      /psyng/swrrshdr-bname = l_current_user."sy-uname. C0700
    WHEN 'BACK'.
      SET SCREEN 0.
      LEAVE SCREEN.
    WHEN 'LCL_SYS'.
