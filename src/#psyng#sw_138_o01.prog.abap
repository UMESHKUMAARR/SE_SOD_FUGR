...
*---enable/desable screen field
  LOOP AT SCREEN.
    IF screen-name = '/PSYNG/SWRESHDR-SODVRSIO'.
      IF g_fltr_all_vrsio = 'X'.
        screen-input = 0.
        CLEAR /psyng/swreshdr-sodvrsio.
      ELSE.
        screen-input = 1.
      ENDIF.
      MODIFY SCREEN.
    ...
    IF screen-name = '/PSYNG/SWRESHDR-SETID'.
      IF  g_fltr_all_set = 'X'.
        screen-input = 0.
        CLEAR /psyng/swreshdr-setid.
      ELSE.
        screen-input = 1.
      ENDIF.
      MODIFY SCREEN.
...
*         IMPORTING
*              e_vrsio = /psyng/swreshdr-sodvrsio.
    /psyng/swreshdr-sodvrsio = g_vrsio.
*--default to the latest published configuration set (highest nr)
    CALL FUNCTION '/PSYNG/SW_CGF_SET_DEFAULT'
         EXPORTING
              i_vrsio      = 'X'
              i_cfgvrsio   = /psyng/swreshdr-sodvrsio
         IMPORTING
              e_config_set = /psyng/swreshdr-setid.
    G_FLTR_ALL_SET = 'X'."default to all config sets
    g_loaded_first = 'X'.
  ENDIF.
ENDMODULE.
