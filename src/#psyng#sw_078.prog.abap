REPORT /psyng/sw_078 MESSAGE-ID /psyng/sw.

TABLES : /psyng/sw_exlltx.

DATA : BEGIN OF gs_tc_exclude .
DATA:   sel(1) TYPE c.
        INCLUDE STRUCTURE /psyng/sw_exlltx.
...
**Authorization check for Display table /psyng/sw_exlltx
**SF 1665
AUTHORITY-CHECK OBJECT 'S_TABU_DIS'
           ID 'ACTVT' FIELD '03'
           ID 'DICBERCLS' FIELD 'Y&S5'.
  if sy-subrc ne 0.
     MESSAGE e113(/psyng/sw) WITH TEXT-e04 '/psyng/sw_exlltx'.
    stop.
  endif.
**SF 1665
**********************************
  ...
  PERFORM enqueue.
  SELECT * FROM /psyng/sw_exlltx INTO CORRESPONDING FIELDS OF
    TABLE gt_tc_exclude.
  CALL SCREEN 100.
...
ENDMODULE.                 " TC_EXCLUDE_change_tc_attr  OUTPUT
*&---------------------------------------------------------------------*
...
ENDMODULE.                 " TC_EXCLUDE_get_lines  OUTPUT
*&---------------------------------------------------------------------*
...
MODULE tc_exclude_modify INPUT.
  ...
  gf_data_change = 'X'.
ENDMODULE.                 " TC_EXCLUDE_modify  INPUT
*&---------------------------------------------------------------------*
...
ENDMODULE.                 " TC_EXCLUDE_mark  INPUT
*&---------------------------------------------------------------------*
...
ENDMODULE.                 " TC_EXCLUDE_user_command  INPUT
**&---------------------------------------------------------------------
**
**&      Form  user_ok_tc
**&---------------------------------------------------------------------
...
**----------------------------------------------------------------------
**
*FORM user_ok_tc USING    VALUE(P_0263)
*                         VALUE(P_0264)
*                         VALUE(P_0265)
*                CHANGING P_OK_CODE.
*
...
**SF 1665
    AUTHORITY-CHECK OBJECT 'S_TABU_DIS'
     ...
     clear sy-ucomm.
         ...
         MESSAGE e113(/psyng/sw) WITH TEXT-e05 '/psyng/sw_exlltx'.
        stop.
      endif.

  DATA : ls_excl TYPE /psyng/sw_excltx,
           lt_exclude TYPE TABLE OF /psyng/sw_exlltx.
* Since there's not much data, delete everything and re-add
*BOC:HBHALLA (17/12/24)
  DELETE FROM /psyng/sw_exlltx WHERE sign NE space.
...
*Save changes to database
  MODIFY /psyng/sw_exlltx FROM TABLE lt_exclude.
  COMMIT WORK.
  IF sy-subrc = 0.
...
"#EC SAST_CI_GEN_CHECK (HBHALLA)
    EXPORTING
      ...
      default_filename  = 'SW_exlltx.csv'
...
"#EC SAST_CI_GEN_CHECK (HBHALLA)
      EXPORTING
        window_title      = title
        default_extension = 'csv'
        default_file_name = 'SW_exlltx.csv'
        file_filter       = '*.csv'
      CHANGING
        filename          = fn
        path              = path
...
*EOC:HBHALLA (097)
  LOOP AT lt_import INTO l_import.
    ...
    IF   gs_tc_exclude-type = 'EQ' OR
      ...
      IF gs_tc_exclude-sign = 'I' OR
         gs_tc_exclude-sign = 'E'.
        READ TABLE gt_tc_exclude WITH KEY
          type = gs_tc_exclude-type
          low = gs_tc_exclude-low
          high = gs_tc_exclude-high
          TRANSPORTING NO FIELDS.
        IF sy-subrc <> 0.
          APPEND  gs_tc_exclude TO gt_tc_exclude.
        ENDIF.
      ELSE.
        MESSAGE e113 WITH text-e01 text-e03 gs_tc_exclude-sign.
      ENDIF.
    ELSE.
      MESSAGE e113 WITH text-e01 text-e02 gs_tc_exclude-type.
    ENDIF.
  ENDLOOP.
