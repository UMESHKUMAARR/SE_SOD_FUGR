*----------------------------------------------------------------------*
* Function Module       : /PSYNG/SW_USR_CRI_AUTH_DATA_NG
...
*----------------------------------------------------------------------*
FUNCTION /psyng/sw_usr_cri_auth_data_ng.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
CONSTANTS: lc_fname TYPE rs38l_fnam
        VALUE '/PSYNG/SW_USR_CRI_AUTH_DATA_NG'.
...
*  SELECT agr_name profile INTO      "get all profiles at once
...
*              INTO CORRESPONDING FIELDS OF TABLE ustcodeauth
...
*  REFRESH ustcodeauth.
  ...
  SELECT object auth field low high
         INTO (wa_ust12-objct, wa_ust12-auth, wa_ust12-field,
               wa_ust12-von, wa_ust12-bis)
         FROM agr_1251
         WHERE agr_name = role and
               object = 'S_TCODE' AND
               deleted <> 'X'.
    wa_ust12-aktps = 'A'.
    INSERT wa_ust12 INTO TABLE dbust12.
*    APPEND dbust12.
  ENDSELECT.
  ...
  PERFORM fill_cri_usertcode_ng
            TABLES ftcodes USING '000000000000' dest.

  PERFORM fill_cri_userauth_ng USING role dest.
  userauth[] = suserauth[].
  REFRESH: suserauth.
...
**----------------------------------------------------------------------
**
**FORM fill_cri_userprof TABLES userprof STRUCTURE /psyng/userprof
...
*    ENDLOOP.
**    SORT userprof.
**    DELETE ADJACENT DUPLICATES FROM userprof COMPARING ALL FIELDS.
**    USERPROF_FM[] = USERPROF[].
*  ENDSELECT.
*ENDFORM.
...
*  <--  p2        text
*----------------------------------------------------------------------*
*FORM fill_cri_usertcode TABLES usertcode STRUCTURE /psyng/usertcode
*                           userprof  STRUCTURE /psyng/userprof
*                           ftcodes   STRUCTURE ftcodes
*                           USING userid LIKE usr02-bname
*                                 dest LIKE rfcdes-rfcdest.
FORM fill_cri_usertcode_ng TABLES ftcodes   STRUCTURE ftcodes
                           USING userid LIKE usr02-bname
                                 dest LIKE rfcdes-rfcdest.
...
*                             auth = ust10s-auth AND
                        aktps = 'A' AND
                         field = 'TCD'.
    MOVE-CORRESPONDING dbust12 TO wa_iiust12.
    INSERT wa_iiust12 INTO TABLE iiust12.
  ENDLOOP.

  LOOP AT iiust12.
    fr_low = iiust12-von.
    to_high = iiust12-bis.
    PERFORM fill_cri_transaction_ng
                    TABLES ftcodes USING userid.
  ENDLOOP.
*    ENDSELECT.
*  ENDLOOP.
*  SORT usertcode.
*  DELETE ADJACENT DUPLICATES FROM usertcode COMPARING ALL FIELDS.
*  USERTCODE_FM[] = USERTCODE[].
*  REFRESH USERPROF.
ENDFORM.                    " FILL_USERTCODE

*---------------------------------------------------------------------*
*       FORM fill_cri_userauth_ng                                     *
*---------------------------------------------------------------------*
*       ........                                                      *
*---------------------------------------------------------------------*
*  -->  ROLE                                                          *
*  -->  DEST                                                          *
*---------------------------------------------------------------------*
FORM fill_cri_userauth_ng USING role LIKE agr_define-agr_name
                                dest LIKE rfcdes-rfcdest.
  SELECT object auth
         INTO (wa_suserauth-objct, wa_suserauth-auth)
         FROM agr_1250
         WHERE agr_name = role AND
               deleted <> 'X'.
**** Below gets just the authorization name
    wa_suserauth-bname   = '000000000000'.
    CONCATENATE sy-sysid sy-mandt INTO wa_suserauth-rfcdest.
    wa_suserauth-agr_name = role.
...
*                             ftcodes   STRUCTURE ftcodes
*                             USING userid LIKE usr02-bname.
FORM fill_cri_transaction_ng TABLES ftcodes   STRUCTURE ftcodes
                             USING userid LIKE usr02-bname.

  DATA:   iagrprof_idx TYPE i,
          l_tcode TYPE tstct-tcode.
  DATA: BEGIN OF lt_tstct OCCURS 0,
  ...
  ELSE.
    IF fr_low > space AND to_high > space. "If auth has range
      ...
      SELECT tcode FROM tstct INTO TABLE lt_tstct
          WHERE tcode >= fr_low AND tcode <= to_high AND
                sprsl = sy-langu.
      LOOP AT lt_tstct.
        READ TABLE ftcodes
                     WITH KEY tcode = lt_tstct-tcode BINARY SEARCH
                                            TRANSPORTING NO FIELDS.
        CHECK sy-subrc = 0.
        wa_susertcode-bname = userid.
        wa_susertcode-tcode = lt_tstct-tcode.
        wa_susertcode-auth  = iiust12-auth.
        wa_susertcode-profn = ust10s-profn.
        CONCATENATE sy-sysid sy-mandt INTO wa_susertcode-rfcdest.
      ...
      IF sy-subrc = 0.                    "get all transactions begining
          ...
          LOOP AT lt_tstct.
          READ TABLE ftcodes WITH KEY tcode = lt_tstct-tcode
                                                BINARY SEARCH
                                                TRANSPORTING NO FIELDS.
          CHECK sy-subrc = 0.
          wa_susertcode-bname = userid.
          ...
          IF sy-subrc = 0.
            wa_susertcode-agr_name = iagrprof-agr_name.
          ENDIF.
*          APPEND usertcode.  CLEAR usertcode.
          INSERT wa_susertcode INTO TABLE susertcode.
          CLEAR wa_susertcode.
         ENDLOOP.
      ENDIF.
...
*          APPEND usertcode.  CLEAR usertcode.
      ...
      ELSE.                "If specific transaction is specified
        ...
        FROM tstct WHERE tcode = iiust12-von AND
                                         sprsl = sy-langu.
