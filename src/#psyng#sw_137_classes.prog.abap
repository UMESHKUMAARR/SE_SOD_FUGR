...
*--NODE_KEY type TV_NODEKEY
    ...
    IF sy-subrc = 0.
      CASE ls_item-text.
        ...
        WHEN 'BNAME'.
          PERFORM expand_bname    USING node_key.
        WHEN 'USERCON'.
          PERFORM expand_usercon  USING node_key.
        WHEN 'CONFUN'.
  ...
  METHOD handle_link_click.
    DATA : l_node_type TYPE string,
            ...
            line(80),
             ...
             lv_item TYPE c LENGTH 72, "HBHALLA
             lv_len TYPE i,            "HBHALLA
             lv_sysindex TYPE /PSYNG/SYSID, "HBHALLA
             ls_system LIKE LINE OF gt_system, "HBHALLA
             l_rest     type string.
...
*--system
    ...
    IF NOT lt_swresisys[] IS INITIAL.
      ...
      WHERE systid = lt_swresisys-sysid.
    ENDIF.
    CONCATENATE sy-sysid sy-mandt INTO l_local.
    SELECT SINGLE sodvrsio FROM /psyng/swreshdr INTO (l_sodvrsio)
       WHERE aid = g_aid.
*--Handle link clicks
    ...
    CASE l_nodetype.
      WHEN 'B'. "User
        ...
        CALL FUNCTION 'POPUP_TO_DECIDE_WITH_MESSAGE'
             EXPORTING
                  defaultoption     = '1'
                  diagnosetext1     = text-055
                  diagnosetext2     = text-056
                  textline1         = text-123
                  text_option1      = text-124
                  text_option2      = text-125
                  icon_text_option1 = 'ICON_TBH'
                  icon_text_option2 = 'ICON_OVERVIEW'
                  titel             = text-126
                  cancel_display    = 'X'
             IMPORTING
                  answer            = answer.
...
*Begin of Addition:HBHALLA(CVA_PR2_Static txn call)(05/05/26)
        CALL FUNCTION 'AUTHORITY_CHECK_TCODE'
         ...
         EXCEPTIONS
           ...
           OTHERS        = 3.
            IF sy-subrc = 1.
              SET PARAMETER ID 'XUS' FIELD ls_item-text.
...
*--- prepare selection screen

            DATA : lt_iseltab  TYPE STANDARD TABLE OF  rsparams,
                   ls_iseltab TYPE rsparams.
            REFRESH lt_iseltab.
            ls_iseltab-selname = 'USERLIST'.       "Call from Summary
            ls_iseltab-kind    = 'S'.
            ls_iseltab-sign    = 'I'.
            ls_iseltab-option  = 'EQ'.
            ls_iseltab-low     = ls_item-text.
            APPEND ls_iseltab TO lt_iseltab.

            ls_iseltab-selname = 'USRTYPE'.
            ls_iseltab-kind    = 'S'.
            ls_iseltab-sign    = 'I'.
            ls_iseltab-option  = 'EQ'.
            ls_iseltab-low     = 'A'.
            ...
            LOOP AT lt_system INTO ls_sys.
              IF ls_sys-systid <> l_local.
                ls_iseltab-selname = 'XUSRRFC'.
                ls_iseltab-kind    = 'S'.
                ls_iseltab-sign    = 'I'.
                ls_iseltab-option  = 'EQ'.
                ls_iseltab-low     = ls_sys-rfcdest.
                APPEND ls_iseltab TO lt_iseltab.
              ELSE.
                lf_local = 'X'.
              ENDIF.
            ENDLOOP.
            IF lf_local IS INITIAL.
              ls_iseltab-selname = 'P_REMONL'.
              ls_iseltab-kind    = 'P'.
              ls_iseltab-sign    = 'I'.
              ls_iseltab-option  = 'EQ'.
              ls_iseltab-low     = 'X'.
              APPEND ls_iseltab TO lt_iseltab.
            ENDIF.
            ls_iseltab-selname = 'P_LOCAL'.
            ls_iseltab-kind    = 'P'.
            ls_iseltab-sign    = 'I'.
            ls_iseltab-option  = 'EQ'.
            ls_iseltab-low     = 'X'.
            APPEND ls_iseltab TO lt_iseltab.
            ls_iseltab-selname = 'P_REMOTE'.
            ls_iseltab-kind    = 'P'.
            ls_iseltab-sign    = 'I'.
            ls_iseltab-option  = 'EQ'.
            ls_iseltab-low     = 'X'.
            APPEND ls_iseltab TO lt_iseltab.
            ls_iseltab-selname = 'P_CROSS'.
            ls_iseltab-kind    = 'P'.
            ls_iseltab-sign    = 'I'.
            ls_iseltab-option  = 'EQ'.
            ls_iseltab-low     = 'X'.
...
*BOC:HBHALLA (02/06/25) (PN-13314) (Issue2)
            CLEAR ls_item.
...
*                       item_name = 'SYSINDEX'.
              ...
              WITH KEY node_key = ls_node-node_key
                       item_name = 'HEADER'.
                 IF sy-subrc = 0.
                     lv_len = strlen( ls_item-text ) - 2.
                     lv_item = ls_item-text+0(lv_len).

                     READ TABLE gt_system INTO ls_system
                                WITH KEY sysid = lv_item.
                          IF sy-subrc = 0.
                            lv_sysindex = ls_system-sysindex.
                          ENDIF.
                 ENDIF.
            ENDIF.
            PERFORM display_role_in_pfcg USING l_agr_name lv_sysindex.
***   PERFORM display_role_in_pfcg USING l_agr_name ls_item-text.
*EOC:HBHALLA (02/06/25) (PN-13314) (Issue2)
          WHEN '2'.
...
CALL FUNCTION 'PRGN_SHOW_EDIT_AGR' "#EC SAST_CI_GEN_CHECK (HBHALLA)
    ...
    WHEN '2'.
      ...
      READ TABLE gt_system WITH KEY sysindex = gt_system-sysindex.
      IF sy-subrc = 0.
        READ TABLE gt_sysinfo WITH KEY systid = gt_system-sysid.
        IF sy-subrc = 0.
