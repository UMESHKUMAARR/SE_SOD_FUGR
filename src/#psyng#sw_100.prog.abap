...
*&---------------------------------------------------------------------*
REPORT /psyng/sw_100 MESSAGE-ID /psyng/sw.
...
DATA: go_alvgrid        TYPE REF TO cl_gui_alv_grid,
      ...
      gt_mctype         TYPE TABLE OF /psyng/sw_mctype,
...
*---------------------------------------------------------------------*
  METHOD data_changed.
    DATA: ls_mod_cells TYPE lvc_s_modi,
          ...
          ls_mctype    LIKE LINE OF gt_mctype.
    ...
    LOOP AT er_data_changed->mt_mod_cells INTO ls_mod_cells
      ...
      LOOP AT gt_mctype INTO ls_mctype WHERE type = ls_mod_cells-value.
  ...
  ELSE.
    ...
    CALL METHOD go_alvgrid->refresh_table_display
      EXCEPTIONS
        ...
        OTHERS         = 2.
  ENDIF.
*BOC:HBHALLA (PN-15537) (19/09/25) (Insert/Delete missing)
  IF gf_dispchg = gc_display.
  ...
  ELSE.
    CALL METHOD go_alvgrid->set_ready_for_input
      EXPORTING
        i_ready_for_input = 1.
  ENDIF.
*EOC:HBHALLA (PN-15537) (19/09/25) (Insert/Delete missing)
ENDFORM.                    " display_alv
...
*----------------------------------------------------------------------*
FORM get_data.
  SELECT * INTO TABLE gt_mctype FROM /psyng/sw_mctype.
ENDFORM.                    " get_data
...
FORM prepare_field_catalog TABLES   et_fieldcat STRUCTURE lvc_s_fcat.
  ...
  CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
       EXPORTING
            i_structure_name       = '/PSYNG/SW_MCTYPE'
       CHANGING
...
*BOC:HBHALLA (17/12/24)
  DELETE FROM /psyng/sw_mctype  WHERE TYPE NE SPACE.
"#EC SAST_CI_GEN_CHECK
*EOC:HBHALLA (17/12/24)
  DELETE gt_mctype WHERE type = space.
  INSERT /psyng/sw_mctype FROM TABLE gt_mctype.
  MESSAGE s120.
  COMMIT WORK.
ENDFORM.                    " save_data
...
FORM check_authorization.
  ...
  IF sy-subrc <> 0.
    IF gf_dispchg = gc_display.
      MESSAGE e108(/psyng/sw) WITH l_msg 'Mitigation Types'(003).
    ELSE.
      gf_dispchg = gc_display.
      MESSAGE i108(/psyng/sw) WITH l_msg 'Mitigation Types'(003).
    ENDIF.
  ENDIF.
