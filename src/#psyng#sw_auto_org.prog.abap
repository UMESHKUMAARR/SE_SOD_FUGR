...
DATA :
       ...
       gf_rd_installed type flag,
       g_rd_vrsio      type /psyng/prog_vrsio,
       gf_initialized  type flag.
...
CONSTANTS :
  ...
  c_fm_001               TYPE rs38l_fnam VALUE '/PSYNG/SW_AO_001',
  c_fm_002               TYPE rs38l_fnam VALUE '/PSYNG/SW_AO_002',
  c_fm_003               TYPE rs38l_fnam VALUE '/PSYNG/SW_AO_003',
  c_fm_004               TYPE rs38l_fnam VALUE '/PSYNG/RD_007',
  c_tcode_deriver        TYPE xutcode    value '/PSYNG/DERIVER'
.
...
SELECTION-SCREEN: BEGIN OF BLOCK rd_o WITH FRAME TITLE text-t09 .
PARAMETERS : p_rdclr AS CHECKBOX modif id rd.
PARAMETERS : p_rdupd AS CHECKBOX modif id rd.
SELECTION-SCREEN : BEGIN OF LINE .
SELECTION-SCREEN PUSHBUTTON  2(25) text-t10 USER-COMMAND rdcurrent
                 modif id rd.
SELECTION-SCREEN : END OF LINE.
SELECTION-SCREEN: END OF BLOCK rd_o.
...
AT SELECTION-SCREEN.
  ...
  ENDIF.
  IF sy-ucomm = 'RDCURRENT'.
    AUTHORITY-CHECK OBJECT 'S_TCODE'
             ID 'TCD' FIELD c_tcode_deriver.
    if sy-subrc = 0.
      call transaction c_tcode_deriver."#EC PATHLOCK_CI_DYN_ACCES
    else.
*   You are not authorized to & & & &
    MESSAGE e108(/psyng/sw) WITH 'use' 'Role Deriver' '' ''.
    endif.
  ENDIF.
...
CALL FUNCTION '/PSYNG/BASIS_GET_MODULES'
 EXPORTING
   I_MODULE               = 'DERIVER'
 IMPORTING
   E_INSTALLED            = gf_rd_installed
   E_MODULE_VERSION       = g_rd_vrsio.
  if g_rd_vrsio < '3.0PS2'.
*--This functionality is only supported from RD version 3.0PS2
    clear gf_rd_installed.
  endif.
...
*--Validate RFC Destinations
  CALL FUNCTION '/PSYNG/BC_VALIDATE_RFCDEST'
       EXPORTING
            i_popup       = 'X'
            i_module      = 'SE'
*BOC UMITTAL PN18085 31/03/2026
*            I_MIN_VERSION = '3.1'
*EOC UMITTAL PN18085 31/03/2026
       IMPORTING
            e_cancel   = l_cancel
...
*     You are not authorized to & & & &
  ...
  ELSEIF sy-ucomm = 'RDPROC'.
    PERFORM process_configuration_rd.
    SET SCREEN 0.
    LEAVE SCREEN.
...
*---------------------------------------------------------------------*
MODULE user_command_0002 INPUT.
*BOC UMITTAL 14/01/2025 ATC checks PN 11269
CONSTANTS : lc_ref(7)  VALUE 'REFRESH',
            lc_cur(7)  VALUE 'CURRENT',
            lc_rdcur(9) VALUE 'RDCURRENT',
            lc_rdproc(6) VALUE 'RDPROC',
...
*  ELSEIF sy-ucomm = 'CURRENT'.
  ELSEIF sy-ucomm = lc_CUR.
    SUBMIT /psyng/sw_050 AND RETURN.
*  ELSEIF sy-ucomm = 'RDCURRENT'.
  ELSEIF sy-ucomm = lc_rdcur.
    AUTHORITY-CHECK OBJECT 'S_TCODE'
             ID 'TCD' FIELD c_tcode_deriver.
    if sy-subrc = 0.
*BOC UMITTAL SE VF scan changes-25/11/2024
        call transaction c_tcode_deriver."#EC PATHLOCK_CI_DYN_ACCES
*        call transaction '/PSYNG/DERIVER'.
*EOC UMITTAL SE VF scan changes-25/11/2024
    else.
*     You are not authorized to & & & &
      MESSAGE e108(/psyng/sw) WITH 'use' 'Role Deriver' '' ''.
...
DATA : lt_swsodorgm_in  TYPE TABLE OF /psyng/swsodorgm WITH HEADER LINE,
  ...
  PERFORM load_rfc
          TABLES s_rfc
                 gt_rfcdes
          USING p_remonl.
  CALL FUNCTION c_fm_004 "#EC PATHLOCK_CI_DYN_ACCES
       EXPORTING
            i_update_table = p_rdupd
            i_clear_table  = p_rdclr
       TABLES
            it_swsodorgm   = lt_swsodorgm_in.
  IF p_rdupd = 'X'.
    EXELOG sy-repid 'APPLY_DERIVER'. "#EC PATHLOCK_CI_DYN_ACCES
    MESSAGE s002(/psyng/sw) WITH
    'Org level Values Configuration applied to'(m01)
    'Role Deriver'(m03).
  ENDIF.
...
*--First record contains the counts for ALL company codes.
  ...
  IF p_ekorg = 'X'.
*    count_all_occurences 'EKORG' l_cnt.
...
*    gt_output_temp[] = gt_output[].
  ...
  ENDIF.
  IF p_werks = 'X'.
*    count_all_occurences 'WERKS' l_cnt.
*    gt_output_temp[] = gt_output[].
  ...
  ENDIF.
  IF p_vkorg = 'X'.
*    count_all_occurences 'VKORG' l_cnt.
*    gt_output_temp[] = gt_output[].
  ...
  ENDIF.
  IF p_gsber = 'X'.
*    count_all_occurences 'GSBER' l_cnt.
*    gt_output_temp[] = gt_output[].
  ...
  ENDIF.
  IF p_spart = 'X'.
*    count_all_occurences 'SPART' l_cnt.
*    gt_output_temp[] = gt_output[].
  ...
  ENDIF.
  IF p_vtweg = 'X'.
*    count_all_occurences 'VTWEG' l_cnt.
*    gt_output_temp[] = gt_output[].
  ...
  ENDIF.
  IF p_kkber = 'X'.
*    count_all_occurences 'KKBER' l_cnt.
*    gt_output_temp[] = gt_output[].
  ...
  ENDIF.
  IF p_kokrs = 'X'.
*    count_all_occurences 'KOKRS' l_cnt.
*    gt_output_temp[] = gt_output[].
...
*    ENDIF.
    ...
    IF p_ekorg = 'X'.
*    count_company_occurences lt_bukrs-abb 'EKORG' l_cnt lf_unckeched.
  count_company_occurences lt_bukrs-abb 'EKORG' l_cnt_disp lf_unckeched
    ...
    ENDIF.
    IF p_werks = 'X'.
*    count_company_occurences lt_bukrs-abb 'WERKS' l_cnt lf_unckeched.
  count_company_occurences lt_bukrs-abb 'WERKS' l_cnt_disp lf_unckeched
    ...
    ENDIF.
    IF p_vkorg = 'X'.
*      count_company_occurences lt_bukrs-abb 'VKORG' l_cnt lf_unckeched.
  count_company_occurences lt_bukrs-abb 'VKORG' l_cnt_disp lf_unckeched
    ...
    ENDIF.
    IF p_gsber = 'X'.
*      count_company_occurences lt_bukrs-abb 'GSBER' l_cnt lf_unckeched.
  count_company_occurences lt_bukrs-abb 'GSBER' l_cnt_disp lf_unckeched
    ...
    ENDIF.
    IF p_spart = 'X'.
*      count_company_occurences lt_bukrs-abb 'SPART' l_cnt lf_unckeched.
  count_company_occurences lt_bukrs-abb 'SPART' l_cnt_disp lf_unckeched
    ...
    ENDIF.
    IF p_vtweg = 'X'.
*      count_company_occurences lt_bukrs-abb 'VTWEG' l_cnt lf_unckeched.
  count_company_occurences lt_bukrs-abb 'VTWEG' l_cnt_disp lf_unckeched
    ...
    ENDIF.
    IF p_kkber = 'X'.
*      count_company_occurences lt_bukrs-abb 'KKBER' l_cnt lf_unckeched.
  count_company_occurences lt_bukrs-abb 'KKBER' l_cnt_disp lf_unckeched
    ...
    ENDIF.
    IF p_kokrs = 'X'.
*      count_company_occurences lt_bukrs-abb 'KOKRS' l_cnt lf_unckeched.
...
*    dyn_value  'MANUAL' lt_bukrs-manual <fs_o>.
    IF lf_manual = 'X'.
