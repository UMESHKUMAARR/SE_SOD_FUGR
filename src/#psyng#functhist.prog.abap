...
*
*----------------------------------------------------------------------*
TABLES: /psyng/function, /psyng/history.
...
DATA: BEGIN OF gt_fun_his OCCURS 0,
      ...
      sodvrsio LIKE /psyng/function-vrsio,
      ...
      sodvrsio(3) , "LIKE /PSYNG/FUNCTION-VRSIO,
      END OF gt_tran_his.
...
PARAMETERS    :  sodvrsio LIKE /psyng/function-vrsio OBLIGATORY.
...
AT SELECTION-SCREEN ON HELP-REQUEST FOR function.

  PERFORM show_help USING '/PSYNG/FUNCTHIST_FUNCTION'.
...
*EOC UMITTAL SE VF scan changes-25/11/2024
  SELECT oldval status create_usr create_dat create_tim vrsio
  FROM /psyng/history
  INTO TABLE gt_fun_his
  ...
  IF NOT gt_fun_his[] IS INITIAL.


    SELECT oldval newval hdrfld status create_usr create_dat create_tim
         FROM /psyng/history INTO CORRESPONDING FIELDS OF TABLE gt_tran
                            FOR ALL ENTRIES IN gt_fun_his
                            WHERE tabname = '/PSYNG/FUNCTTRAN'
                                AND   hdrfld  = gt_fun_his-oldval
                               AND   create_dat = gt_fun_his-create_dat
                               AND   create_tim = gt_fun_his-create_tim
                                AND   vrsio      = sodvrsio
                               AND   create_usr = gt_fun_his-create_usr.
  ...
  LOOP AT gt_fun_his.
    LOOP AT gt_tran WHERE hdrfld = gt_fun_his-oldval AND create_dat =
    gt_fun_his-create_dat AND create_tim = gt_fun_his-create_tim AND
    create_usr = gt_fun_his-create_usr.
      MOVE-CORRESPONDING gt_fun_his TO gt_tran_his.

      READ TABLE gt_user_info WITH KEY bname = gt_fun_his-create_usr.
      IF sy-subrc = 0.
        gt_tran_his-name_text = gt_user_info-name_text.
      ENDIF.
      READ TABLE gt_user_info WITH KEY bname =  gt_tran-create_usr.
      IF sy-subrc = 0.
        gt_tran_his-name_text1 = gt_user_info-name_text.
      ENDIF.


      IF gt_fun_his-status = 'U'.
        gt_tran_his-status = text-024.
      ELSEIF gt_fun_his-status = 'I'.
        gt_tran_his-status = text-025.
      ELSEIF gt_fun_his-status = 'D'.
        gt_tran_his-status = text-026.
      ENDIF.
      gt_tran_his-tcode = gt_tran-oldval.
      IF gt_tran-status = 'U'.
        gt_tran_his-status_t = text-024.
      ELSEIF gt_tran-status = 'I'.
        gt_tran_his-status_t = text-025.
      ELSEIF gt_tran-status = 'D'.
        gt_tran_his-status_t = text-026.
      ENDIF.
      gt_tran_his-create_usr_t = gt_tran-create_usr.
      gt_tran_his-create_dat_t = gt_tran-create_dat.
      gt_tran_his-create_tim_t = gt_tran-create_tim.
      APPEND gt_tran_his.
      CLEAR gt_tran_his.
    ENDLOOP.
    IF sy-subrc <> 0.
  ...
  SORT gt_tran_his BY sodvrsio oldval create_usr create_dat create_tim
      tcode create_usr_t create_dat_t create_tim_t.

  PERFORM build_fieldcat.

  PERFORM build_sort.

  PERFORM out_put_display.
...
FORM build_fieldcat.
 DATA: w_fieldcat TYPE slis_fieldcat_alv.        "Field Catalog Workarea
  ...
  w_fieldcat-tabname   = 'GT_TRAN_HIS'.
...
**  For Function ID **
    WHEN 'OLDVAL'.
      ...
      SELECT SINGLE description FROM /psyng/function INTO line
...
"(++)EOC UMITTAL SE VF scan-25/11/2024.                  .
      ...
      IF sy-subrc = 0.
        ...
        ELSE.
          MESSAGE s398(00) WITH text-009.
        ENDIF.
      ELSE.
        MESSAGE s398(00) WITH text-010.
      ENDIF.
...
*&---------------------------------------------------------------------*
*&      Form  OUT_PUT_DISPLAY
*&---------------------------------------------------------------------*
*       text
*---------------------------------------------------------------------*
FORM out_put_display.
  DATA: y_layout  TYPE slis_layout_alv,            "ALV Report Layout
