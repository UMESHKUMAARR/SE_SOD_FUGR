FUNCTION /PSYNG/SWAUDC_WRITE_DOCUMENT.
*"----------------------------------------------------------------------
*"*"Update function module:
*"
*"*"Local interface:
*"  IMPORTING
*"     VALUE(OBJECTID) LIKE  CDHDR-OBJECTID
*"     VALUE(TCODE) LIKE  CDHDR-TCODE
*"     VALUE(UTIME) LIKE  CDHDR-UTIME
*"     VALUE(UDATE) LIKE  CDHDR-UDATE
*"     VALUE(USERNAME) LIKE  CDHDR-USERNAME
*"     VALUE(PLANNED_CHANGE_NUMBER) LIKE  CDHDR-PLANCHNGNR DEFAULT
*"       SPACE
*"     VALUE(OBJECT_CHANGE_INDICATOR) LIKE  CDHDR-CHANGE_IND DEFAULT
*"       'U'
*"     VALUE(PLANNED_OR_REAL_CHANGES) LIKE  CDHDR-CHANGE_IND DEFAULT
*"       SPACE
*"     VALUE(NO_CHANGE_POINTERS) LIKE  CDHDR-CHANGE_IND DEFAULT SPACE
*"     VALUE(UPD_ICDTXT_SWAUDC) LIKE  CDPOS-CHNGIND DEFAULT SPACE
*"     VALUE(N_PSYNG_SWAUDC) LIKE  /PSYNG/SWAUDC2 STRUCTURE
*"        /PSYNG/SWAUDC2
*"     VALUE(O_PSYNG_SWAUDC) LIKE  /PSYNG/SWAUDC2 STRUCTURE
*"        /PSYNG/SWAUDC2
*"     VALUE(UPD_PSYNG_SWAUDC) LIKE  CDPOS-CHNGIND DEFAULT SPACE
*"  TABLES
*"      ICDTXT_SWAUDC STRUCTURE  CDTXT
*"----------------------------------------------------------------------
data objclass like CDHDR-OBJECTCLAS.
data tabname like CDPOS-TABNAME.
objclass = '/PSYNG/SWAUDC   '.
tabname = '/PSYNG/SWAUDC2                 '.
CALL FUNCTION 'CHANGEDOCUMENT_OPEN'
  EXPORTING OBJECTCLASS = objclass
            OBJECTID    = OBJECTID
            PLANNED_CHANGE_NUMBER = PLANNED_CHANGE_NUMBER
            PLANNED_OR_REAL_CHANGES = PLANNED_OR_REAL_CHANGES
  EXCEPTIONS SEQUENCE_INVALID = 1
             OTHERS           = 2.

CASE SY-SUBRC.
  WHEN 1. MESSAGE A600 WITH 'SEQUENCE INVALID'.
  WHEN 2. MESSAGE A600 WITH 'OPEN ERROR'.
ENDCASE.


IF UPD_PSYNG_SWAUDC                    NE SPACE.
   CALL FUNCTION 'CHANGEDOCUMENT_SINGLE_CASE'
     EXPORTING  TABLENAME           = tabname
                WORKAREA_OLD        = O_PSYNG_SWAUDC
                WORKAREA_NEW        = N_PSYNG_SWAUDC
                CHANGE_INDICATOR    = UPD_PSYNG_SWAUDC
                DOCU_DELETE         = ' '
     EXCEPTIONS NAMETAB_ERROR       = 1
                OPEN_MISSING        = 2
                POSITION_INSERT_FAILED = 3
                OTHERS              = 4.

   CASE SY-SUBRC.
     WHEN 1. MESSAGE A600 WITH 'NAMETAB-ERROR'.
     WHEN 2. MESSAGE A600 WITH 'OPEN MISSING'.
     WHEN 3. MESSAGE A600 WITH 'INSERT ERROR'.
     WHEN 4. MESSAGE A600 WITH 'SINGLE ERROR'.
   ENDCASE.
ENDIF.


IF UPD_ICDTXT_SWAUDC           NE SPACE.
   CALL FUNCTION 'CHANGEDOCUMENT_TEXT_CASE'
     TABLES    TEXTTABLE  = ICDTXT_SWAUDC
     EXCEPTIONS OPEN_MISSING           = 1
                POSITION_INSERT_FAILED = 2
                OTHERS                 = 3.

   CASE SY-SUBRC.
     WHEN 1. MESSAGE A600 WITH 'OPEN MISSING'.
     WHEN 2. MESSAGE A600 WITH 'INSERT ERROR'.
     WHEN 3. MESSAGE A600 WITH 'TEXT ERROR'.
   ENDCASE.
ENDIF.


CALL FUNCTION 'CHANGEDOCUMENT_CLOSE'
  EXPORTING  OBJECTCLASS    = objclass "'/PSYNG/SWAUDC   '
             OBJECTID       = OBJECTID
             DATE_OF_CHANGE = UDATE
             TIME_OF_CHANGE = UTIME
             TCODE          = TCODE
             USERNAME       = USERNAME
OBJECT_CHANGE_INDICATOR = OBJECT_CHANGE_INDICATOR
NO_CHANGE_POINTERS           = NO_CHANGE_POINTERS
  EXCEPTIONS HEADER_INSERT_FAILED   = 1
             OBJECT_INVALID         = 2
             OPEN_MISSING           = 3
             NO_POSITION_INSERTED   = 4
             OTHERS                 = 5.

CASE SY-SUBRC.
  WHEN 1. MESSAGE A600 WITH 'INSERT HEADER FAILED'.
  WHEN 2. MESSAGE A600 WITH 'OBJECT INVALID'.
  WHEN 3. MESSAGE A600 WITH 'OPEN MISSING'.
  WHEN 5. MESSAGE A600 WITH 'CLOSE ERROR'.
ENDCASE.

ENDFUNCTION.
