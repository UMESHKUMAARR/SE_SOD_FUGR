...
*----------------------------------------------------------------------*
FUNCTION /psyng/sw_api_i_general_r_info.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  DATA : lt_return     TYPE TABLE OF  bapiret2 WITH HEADER LINE,
         ...
         lt_role_ana_header TYPE TABLE OF /psyng/swrrshdr,
         ls_role_ana_header TYPE /psyng/swrrshdr,
...
*--Get Information about the latest published Configuration Set
  ...
  IF if_user_info IS NOT INITIAL OR if_both_info IS NOT INITIAL.
...
*--Get RunID of the latest unrestricted run of Stored SOD results
  ...
  "(++)BOC UMITTAL SE VF scan-25/11/2024
    ...
    IF analysis_run IS INITIAL.
      log lt_return 'W'(004) 'NO_COMPLETE_CHECK'(005)
            'No Analysis of all conflicts for all users found'(006) ''
            '' ''.
    ENDIF.
...
*BOC:HBHALLA (PN-15678) (13/10/25)
  IF if_role_info IS NOT INITIAL OR if_both_info IS NOT INITIAL.
    PERFORM general_role_info_load_all_sys
     TABLES "et_res_systems
            et_rrshdr
            it_systems
     USING
...
*--Get RunID of the latest unrestricted run of Stored SOD results
    IF lines( et_rrshdr ) > 0.
      lt_role_ana_header[] = et_rrshdr[].
      DELETE lt_role_ana_header WHERE (
                                  end_date EQ '00000000' AND
                                  end_time EQ '000000'
                                 )
                                 OR
      ...
      SORT lt_role_ana_header BY aid DESCENDING.
      READ TABLE lt_role_ana_header INTO ls_role_ana_header INDEX 1.
      IF sy-subrc = 0.
        analysis_run = ls_role_ana_header-aid.
        l_sdate      = ls_role_ana_header-start_date.
        IF NOT l_sdate IS INITIAL.
          CALL FUNCTION 'CONVERT_DATE_TO_EXTERNAL'
               EXPORTING
                    date_internal = l_sdate
  ...
  "(++)BOC UMITTAL SE VF scan-25/11/2024
    ...
    IF analysis_run IS INITIAL.
      log lt_return 'W'(004) 'NO_COMPLETE_CHECK'(005)
            'No Analysis of all conflicts for all roles found'(007) ''
            '' ''.
    ENDIF.
