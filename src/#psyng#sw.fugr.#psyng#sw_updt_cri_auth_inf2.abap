...
*----------------------------------------------------------------------*
FUNCTION /PSYNG/SW_UPDT_CRI_AUTH_INF2.
*"----------------------------------------------------------------------
*"*"Local interface:
*"  IMPORTING
...
*"  TABLES
*"      SYSCANDT STRUCTURE  /PSYNG/SW_CRATD2
*"----------------------------------------------------------------------
*BOC:UMITTAL CVA scan fix 27/02/2026
CONSTANTS: lc_fname TYPE rs38l_fnam
        VALUE '/PSYNG/SW_UPDT_CRI_AUTH_INF2'.
...
*EOC:UMITTAL CVA scan fix 27/02/2026
  DATA: wa LIKE /PSYNG/SW_CRATD2,
        count TYPE i,
        del_syscandt1 TYPE /PSYNG/SW_CRATD2 OCCURS 0 WITH HEADER LINE,
        del_syscandt2 TYPE /PSYNG/SW_CRATD2 OCCURS 0 WITH HEADER LINE,
        critauths     TYPE /psyng/swaudhdr OCCURS 0 WITH HEADER LINE,
        syscandt3     TYPE /PSYNG/SW_CRATD2 OCCURS 0 WITH HEADER LINE,
        iusr02        TYPE usr02           OCCURS 0 WITH HEADER LINE,
        wa_iusr02     TYPE usr02,
        syscandt4     TYPE /PSYNG/SW_CRATD2 OCCURS 0 WITH HEADER LINE,
        wa_syscandt4  TYPE /PSYNG/SW_CRATD2,
  ...
  IF nodelete EQ space.          "don't delete all records of users if
    ...
    LOOP AT del_syscandt1.
      SELECT * FROM /PSYNG/SW_CRATD2   "#EC CI_SEL_NESTED
               APPENDING TABLE del_syscandt2  "get all records of
...
* Commit every COMMITCOUNT records
    ...
    IF records LE commitcount.
      DELETE /PSYNG/SW_CRATD2 FROM TABLE del_syscandt2.
      COMMIT WORK.
    ELSE.
      ...
      LOOP AT del_syscandt2.
        ...
        IF loopcount GE commitcount.
          DELETE /PSYNG/SW_CRATD2      "#EC CI_IMUD_NESTED
      ...
      IF NOT del_syscandt1[] IS INITIAL.
        DELETE /PSYNG/SW_CRATD2 FROM TABLE del_syscandt1.
        COMMIT WORK.
        REFRESH del_syscandt1.
...
*************
*  DELETE /PSYNG/SW_CRATD2FROM TABLE del_syscandt2.
*  COMMIT WORK.
*************
  ...
  SELECT * FROM /psyng/swaudhdr INTO TABLE critauths
  WHERE vrsio = vrsio.
  syscandt3[] = syscandt[].

  DELETE syscandt WHERE swaudid = 'ALL'.

  LOOP AT syscandt3 WHERE swaudid = 'ALL'.
    LOOP AT critauths.
      syscandt-agr_name = syscandt3-agr_name.
      syscandt-scandate = sy-datum.
      syscandt-swaudid = critauths-swaudid.
      APPEND syscandt.
    ENDLOOP.
  ENDLOOP.
...
* Commit every COMMITCOUNT records
  ...
  IF records LE commitcount.
*    INSERT /PSYNG/SW_CRATD2 FROM TABLE syscandt.
    MODIFY /PSYNG/SW_CRATD2 FROM TABLE syscandt.
    COMMIT WORK.
  ELSE.
    REFRESH syscandt3. CLEAR syscandt3.
    LOOP AT syscandt.
      ...
      IF loopcount GE commitcount.
*        INSERT /PSYNG/SW_CRATD2 FROM TABLE syscandt3.
        MODIFY /PSYNG/SW_CRATD2           "#EC CI_IMUD_NESTED
...
*      INSERT /PSYNG/SW_CRATD2 FROM TABLE syscandt3.
      MODIFY /PSYNG/SW_CRATD2 FROM TABLE syscandt3.
      COMMIT WORK.
      REFRESH syscandt3.
